...
# ------------------ HTML (Dark Legend Theme) ------------------
HTML = """
<!DOCTYPE html>
<html>
<head>
    <title>⚡ THEW LEGEND WALEED KHAN ⚡</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', 'Poppins', Arial, sans-serif; }
        ...
        body::before {
            ...
            width: 100%;
            height: 100%;
            background-image: repeating-linear-gradient(45deg, rgba(255, 215, 0, 0.03) 0px, rgba(255, 215, 0, 0.03) 2px, rgba(0, 191, 255, 0.03) 2px, rgba(0, 191, 255, 0.03) 4px);
            pointer-events: none;
            z-index: 0;
        }
        ...
        .card.active {
            border-color: #ffd700;
            box-shadow: 0 0 25px rgba(255, 215, 0, 0.15), 0 10px 30px rgba(0,0,0,0.7);
        }
        /* Card-specific border colors */
        .card-token { border-color: #ffd700; }
        .card-token.active { border-color: #ffd700; box-shadow: 0 0 25px rgba(255, 215, 0, 0.2); }
        .card-1 { border-color: #87CEEB; }
        .card-1.active { border-color: #87CEEB; box-shadow: 0 0 25px rgba(135, 206, 235, 0.3); }
        .card-2 { border-color: #90EE90; }
        .card-2.active { border-color: #90EE90; box-shadow: 0 0 25px rgba(144, 238, 144, 0.3); }
        .card-3 { border-color: #FFD700; }
        .card-3.active { border-color: #FFD700; box-shadow: 0 0 25px rgba(255, 215, 0, 0.3); }
        .card-4 { border-color: #ffd700; }
        .card-4.active { border-color: #ffd700; box-shadow: 0 0 25px rgba(255, 215, 0, 0.15); }
        ...
        .card-token .card-header h2 { color: #ffd700; }
        .card-1 .card-header h2 { color: #87CEEB; }
        .card-2 .card-header h2 { color: #90EE90; }
        .card-3 .card-header h2 { color: #ffd700; }
        .card-4 .card-header h2 { color: #ffd700; }
        ...
        .card-token .instruction { color: #ffd700; border-left: 3px solid #ffd700; }
        .card-1 .instruction { color: #87CEEB; border-left: 3px solid #87CEEB; }
        .card-2 .instruction { color: #90EE90; border-left: 3px solid #90EE90; }
        .card-3 .instruction { color: #ffd700; border-left: 3px solid #ffd700; }
        .card-4 .instruction { color: #ffd700; border-left: 3px solid #ffd700; }
        ...
        .task-card .task-row .value.target-val { color: #ffd700; }
        .task-card .task-row .value.file-val { color: #ffd700; }
        .task-card .task-row .value.hater-val { color: #ffd700; }
        .task-card .task-row .value.speed-val { color: #ffd700; }
        .task-card .task-row .value.last-val { color: #ffd700; }
        .task-card .task-row .value.status-val { color: #ffd700; }
        .task-card .task-row .value.uptime-val { color: #ffd700; }
        .task-card .task-row .value.next-val { color: #ffd700; }
    </style>
</head>
<body>
...
<div id="loginModal" class="login-modal">
    <div class="modal-content">
        <div class="bot-banner-container">
            <img src="https://i.ibb.co/sptgF3Fk/df4debb4-4fbb-4a79-82b3-c356a2b4dfdd.jpg" alt="Banner" class="bot-banner-img">
        </div>
        <h2>⚡ THEW LEGEND WALEED KHAN</h2>
...
<div id="taskDetailModal" class="task-detail-modal">
    <div class="modal-content">
        <h2 style="color:#ffd700;">📋 TASK DETAILS</h2>
...
<div class="main-content" id="mainContent">
    <h1>⚡ THEW LEGEND WALEED KHAN</h1>
    <div class="subtitle">
        🍫 TELEGRAM AUTO BOT • THEW LEGEND EDITION 🍫
        ...
        <div class="card card-3" id="card-3">
            ...
            <div class="card-body">
                <div class="form-line">
                    ...
                    <select id="senderAccountSelect" style="color: #ffd700; font-weight: bold;"><option value="">-- NO ACTIVE ACCOUNTS --</option></select>
                </div>
                <div class="form-line">
...
<script>
    // ==================== LOGIN SYSTEM ====================
    // Users: WALEEDKHAN and WALEEDKHAN (both admin)
    const USERS = {
        'WALEEDKHAN': { password: 'WALEEDKHAN', role: 'admin' },
        'WALEEDXD': { password: 'WALEEDXD', role: 'admin' }
    };
        ...
        try {
            ...
            if(data.success && data.groups) {
                ...
                data.groups.forEach(g => {
                    let avatar = g.avatar_url ? `<img src="${g.avatar_url}?t=${Date.now()}" alt="Group">` : `<div style="width:40px;height:40px;border-radius:50%;background:#1a1a2e;display:flex;align-items:center;justify-content:center;color:#ffd700;font-weight:bold;border:2px solid #ffd700;">${g.name.charAt(0)}</div>`;
                    html += `<div class="group-item" onclick="selectGroup('${g.id}', '${g.name.replace(/'/g, "\\\\'")}')">
                                ${avatar}
                                <div class="group-info">
                                    <strong>${g.name}</strong>
                                    <small style="display:block; color:#e0e0e0; margin-top:3px;">ID: ${g.id}</small>
                                </div>
                             </div>`;
                });
    ...
    function selectGroup(id, name) {
        ...
        let selectedHtml = `<div class="selected-group-card">
                                <div>
                                    ...
                                    <small style="color:#e0e0e0;">ID: ${id}</small>
                                </div>
                            </div>`;
    ...
    function openUpdateModal(phone, taskId, currentHater, currentSpeed, currentLastHater, currentTarget) {
        ...
        let avatarHtml = session.avatar_url ? `<img src="${session.avatar_url}?t=${Date.now()}" style="width:36px; height:36px; border-radius:50%; object-fit:cover; border:2px solid #ffd700; margin-right:8px;">` : '';
